/* Constant properties */
var GOORU_REST_ENDPOINT = ('https:' == document.location.protocol ? 'https://' : 'http://') + 'qa.goorulearning.org/gooruapi/rest';
var DOC_HOME = ('https:' == document.location.protocol ? 'https://' : 'http://') + '//qa.goorulearning.org/doc/a/view';
var DOC_CACHE = ('https:' == document.location.protocol ? 'https://' : 'http://') + 'qa';
var HOME_URL = '//sheeban.goorulearning.org/p'
var USER = {
  ...
  var resourcePlayers = {
    ...
    animation: function (resourceUrl, playerContainer) {
      if (resourcePlayers.hasFlashPluginInstall()) {
	    ...
	    url: STATIC_FILE_PATH + '/templates/resources/resourcePlayerInfo.template'
	...
	if (previewValues.questionType == "MC" || previewValues.questionType == "T/F" || previewValues.questionType == 1 || previewValues.questionType == 3) {
	  var resourceInfo = new EJS({
	      url: STATIC_FILE_PATH + '/templates/resources/question/resourceQuestionMCQ.template'
	    }).render({data:previewValues});
	}
	if (previewValues.questionType == "FIB" || previewValues.questionType == 4) {
	  var resourceInfo = new EJS({
	      url: STATIC_FILE_PATH + '/templates/resources/question/resourceQuestionFIB.template'
	    }).render({data:previewValues});
	    console.log(resourceInfo);
	}
	if (previewValues.questionType == "OE" || previewValues.questionType == 6) {
	  var resourceInfo = new EJS({
	      url: STATIC_FILE_PATH + '/templates/resources/question/resourceQuestionOE.template'
	    }).render({data:previewValues});
	}
...
var helper = {
  ...
  userSignin: function(options) { 
	...
	error : function(data) {
      ...
      var resourceInfo = new EJS({url: STATIC_FILE_PATH + '/templates/resources/resourcePlayerInfo.template'}).render({message: key[1] + ' For new API Key, please send request by clicking this link <a href="http://developers.goorulearning.org/request-a-key" target="_blank">Request API Key</a>', reduceSize: 0});
